  ...
  class EventSourceBase extends GObject.Object {
    get isLoading() {
      throw new GObject.NotImplementedError(
        "isLoading in %s".format(this.constructor.name)
      );
    }

    get hasCalendars() {
      throw new GObject.NotImplementedError(
        "hasCalendars in %s".format(this.constructor.name)
      );
    }
    ...
    requestRange(_begin, _end) {
      throw new GObject.NotImplementedError(
        "requestRange in %s".format(this.constructor.name)
      );
    }

    getEvents(_begin, _end) {
      throw new GObject.NotImplementedError(
        "getEvents in %s".format(this.constructor.name)
      );
    }

    hasEvents(_day) {
      throw new GObject.NotImplementedError(
        "hasEvents in %s".format(this.constructor.name)
      );
    }
  }
);
...
const CalendarServerInfo = Gio.DBusInterfaceInfo.new_for_xml(
  CalendarServerIface
);
  ...
  class DBusEventSource extends EventSourceBase {
    ...
    async _initProxy() {
      ...
      this._dbusProxy.connect("notify::g-name-owner", () => {
        if (this._dbusProxy.g_name_owner) this._onNameAppeared();
        else this._onNameVanished();
      });
    ...
    get hasCalendars() {
      if (this._initialized) return this._dbusProxy.HasCalendars;
      else return false;
    }
    ...
    _onEventsRemoved(dbusProxy, nameOwner, argArray) {
      ...
      let changed = false;
      for (const id of ids) changed |= this._events.delete(id);
    ...
    _onClientDisappeared(dbusProxy, nameOwner, argArray) {
      ...
      for (const id of this._events.keys()) {
        if (id.startsWith(sourceUid)) changed |= this._events.delete(id);
      }
    ...
    requestRange(begin, end) {
      if (
        !(
          _datesEqual(begin, this._lastRequestBegin) &&
    ...
    *_getFilteredEvents(begin, end) {
    ...
    getEvents(begin, end) {
      ...
      result.sort((event1, event2) => {
        ...
        let d1 =
          event1.date < begin && event1.end <= end ? event1.end : event1.date;
        let d2 =
          event2.date < begin && event2.end <= end ? event2.end : event2.date;
  ...
  class Calendar extends St.Widget {
    _init() {
      ...
      this._settings.connect(
        "changed::%s".format(SHOW_WEEKDATE_KEY),
    ...
    setDate(date) {
      if (sameDay(date, this._selectedDate)) return;
    ...
    _buildHeader() {
      ...
      this._backButton.add_actor(
        new St.Icon({ icon_name: "pan-start-symbolic" })
      );
      this._topBox.add(this._backButton);
      this._backButton.connect(
        "clicked",
        this._onPrevMonthButtonClicked.bind(this)
      );
      ...
      this._forwardButton.add_actor(
        new St.Icon({ icon_name: "pan-end-symbolic" })
      );
      this._topBox.add(this._forwardButton);
      this._forwardButton.connect(
        "clicked",
        this._onNextMonthButtonClicked.bind(this)
      );
      ...
      for (let i = 0; i < 7; i++) {
        ...
        else col = offsetCols + ((7 + iter.getDay() - this._weekStart) % 7);
    ...
    _onPrevMonthButtonClicked() {
      ...
      if (oldMonth == 0) {
        ...
        if (newDate.getMonth() != oldMonth - 1) {
          let day =
            32 - new Date(newDate.getFullYear(), oldMonth - 1, 32).getDate();
    ...
    _rebuildCalendar() {
      ...
      beginDate.setTime(
        beginDate.getTime() - (weekPadding + daysToWeekStart) * MSECS_IN_DAY
      );
      ...
      while (row < nRows) {
        ...
        if (row == 2) styleClass = "calendar-day-top %s".format(styleClass);

        let leftMost = rtl
          ? iter.getDay() == (this._weekStart + 6) % 7
          : iter.getDay() == this._weekStart;
        if (leftMost) styleClass = "calendar-day-left %s".format(styleClass);

        if (sameDay(now, iter)) styleClass += " calendar-today";
        else if (iter.getMonth() != this._selectedDate.getMonth())
          styleClass += " calendar-other-month-day";

        if (hasEvents) styleClass += " calendar-day-with-events";

        button.style_class = styleClass;

        let offsetCols = this._useWeekdate ? 1 : 0;
        let col;
        if (rtl) col = 6 - ((7 + iter.getDay() - this._weekStart) % 7);
        else col = offsetCols + ((7 + iter.getDay() - this._weekStart) % 7);
        ...
        if (iter.getDay() == this._weekStart) row++;
      }
    ...
    _update() {
      ...
      if (sameYear(this._selectedDate, now))
        this._monthLabel.text = this._selectedDate.toLocaleFormat(
          this._headerFormatWithoutYear
        );
      else
        this._monthLabel.text = this._selectedDate.toLocaleFormat(
          this._headerFormat
...
var NotificationMessage = GObject.registerClass(
  class NotificationMessage extends MessageList.Message {
    _init(notification) {
      ...
      this._updatedId = notification.connect(
        "updated",
    ...
    _disconnectNotificationSignals() {
      if (this._updatedId) this.notification.disconnect(this._updatedId);
      this._updatedId = 0;

      if (this._destroyId) this.notification.disconnect(this._destroyId);
      this._destroyId = 0;
    }
...
var NotificationSection = GObject.registerClass(
  class NotificationSection extends MessageList.MessageListSection {
    ...
    _sourceAdded(tray, source) {
      ...
      obj.notificationAddedId = source.connect(
        "notification-added",
...
var CalendarMessageList = GObject.registerClass(
  class CalendarMessageList extends St.Widget {
    ...
    _addSection(section) {
      ...
      connectionsIds.push(
        section.connect("message-focused", (_s, messageActor) => {
